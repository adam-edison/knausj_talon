...
class ParrotConfig():
    ...
    def setup_mode(self, mode):
        ...
        print(f"  combo_window set to: {self.combo_window}")
    ...
    def execute(self, noise: str):
        ...
        print(f"execute: noise={noise}, base_noises={self.base_noises}")
        if noise not in self.base_noises:
            print(f"  -> noise not in base_noises, returning")
            return
        ...
        print(f"  combo_chain={self.combo_chain}, pending={self.pending_combo}")

        if self.combo_chain in self.delayed_commands:
            print(f"  -> found in delayed_commands")
            ...
            print(f"  -> found combo_chain in immediate_commands")
            ...
            print(f"  -> found single noise in immediate_commands")
...
def parrot_config_noise(sound: str):
    ...
    if parrot_config_saved.parrot_config_user_ref != config:
        print(f"init parrot config: {list(config.keys()) if config else 'empty'}")
        parrot_config_saved.setup(config)
        print(f"  immediate: {list(parrot_config_saved.immediate_commands.keys())}")
        print(f"  delayed: {list(parrot_config_saved.delayed_commands.keys())}")
